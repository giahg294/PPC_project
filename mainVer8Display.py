        ...
        for direction, queue in section_queues.items():
            if not queue.empty():
                ...
                if vehicle['priority'] == -1:
                    try:
                        display_socket.sendall(f"Emergency vehicle from {vehicle['entry']} going to {vehicle['exit']} with priority {vehicle['priority']}, Light: Green\n".encode())
                    except BrokenPipeError:
                        print("[Coordinator] Broken pipe error while sending priority vehicle information.")
                        continue  # 继续下一个车辆
                    print(f"[Coordinator] Emergency vehicle from {vehicle['entry']} going to {vehicle['exit']} has passed through the intersection.")
                ...
                if (vehicle['entry'] in [N, S] and ns == LIGHT_GREEN) or (vehicle['entry'] in [E, W] and we == LIGHT_GREEN):
                    ...
                    try:
                        ...
                        print("[Coordinator] Broken pipe error while sending vehicle information.")
                        continue  # 继续下一个车辆
                    ...
                    try:
                        ...
                        print("[Coordinator] Broken pipe error while sending waiting vehicle information.")
                        continue  # 继续下一个车辆
                    print(f"[Coordinator] Vehicle from {vehicle['entry']} going to {vehicle['exit']} is waiting.")
