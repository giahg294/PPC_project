...
class TrafficLight:
    ...
    def set_normal_state(self, ns_green, we_green):
        ...
        print(f"OUTPUT: 红绿灯变化 - {N}和{S}方向{'绿' if ns_green else '红'}灯，{E}和{W}方向{'绿' if we_green else '红'}灯")
    ...
    # 退出紧急模式，恢复正常信号灯状态 
        with self.lock:
            if self.emergency_count.value > 0:
                self.emergency_count.value = 0  # 强制清零，防止误计
...
def ambulance_gen(section_queues, traffic_light):
    while True:
        time.sleep(random.randint(3, 6))  # 提高生成频率
        entry = random.choice(DIRECTIONS)
        exit = random.choice([d for d in DIRECTIONS if d != entry])
        ...
        print(f"\n !!! 所有车辆注意！ 救护车 {vehicle['license_plate']} 进入 {entry} 方向 ,即将驶向 {exit} 方向，请让行 !!---")
