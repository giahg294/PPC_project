...
请你帮我修改代码：
修改目标
移除 [Display] 对于紧急车辆的输出.在 coordinator 函数中修改对 display_socket.sendall 的调用，让它在检测到紧急车辆时，只打印 [Coordinator] 相关的信息，而不向 display_socket 发送信息。
确保紧急车辆通过后，普通车辆可以继续正常通行：
如果紧急车辆通过后，当前方向的交通灯是绿的，则允许普通车辆通行，不应该让它们继续等待。
...
def vehicle_priority(vehicle):
    ...
    return PRIORITY_RULES[exit].get(entry, 2)  # 查找优先级，默认为最低优先级 (2)
