...
def vehicle_priority(vehicle):
    ...
    # 优先级：1：直行，2：右转，3：左转
    return PRIORITY_RULES[exit].get(entry, 4)  # 查找优先级，默认为最低优先级 (4)
    ...
    while True:
        ...
        for direction in DIRECTIONS:
            ...
            while not section_queues[direction].empty():
                vehicles.append(section_queues[direction].get_nowait())  # 获取队列中的所有车辆（非阻塞方式）
            vehicles.sort(key=lambda v: v["priority"])  # 按优先级排序
            print(f"--------[Coordinator] Vehicles for {direction} priority list: {[(v['entry'], v['priority']) for v in vehicles]}")
