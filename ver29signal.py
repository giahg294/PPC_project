#problem: n'arrive pas a KeyInterrupt via ctrl+c .
...
def ambulance_gen(section_queues, traffic_light):
    while True:
        ...
        #  修改
        # 通过信号通知交通灯进入紧急模式
        os.kill(os.getppid(), signal.SIGUSR1)  # 🔄 向父进程发送信号，触发紧急模式
        ...
        for v in list(section_queues[direction]):
            ...
            if v['type'] == "pripri":
                ...
                if v['priority'] == 1:
                    ...
                    can_pass = no_higher_pri and no_higher_pri_opposite
