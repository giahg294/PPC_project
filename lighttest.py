...
def vehicle_priority(entry, exit_dir):
    # 定义优先级：救护车（在车辆字典中 type 为 "priority"）最高，
    # 其余车辆：直行（优先级 1） > 右转（2） > 左转（3）
    priority_map = {
        N: {S: 1, W: 2, E: 3},
        S: {N: 1, E: 2, W: 3},
        E: {W: 1, N: 2, S: 3},
...
# === display_server：显示进程优化 ===
def display_server(traffic_light):
...
def light_controller(traffic_light, emergency_event):
    ...
    while True:
        if emergency_event.is_set():
            # 当收到紧急通知时，进入紧急模式
            print("light_controller 检测到紧急事件，切换紧急模式")
            ...
            if last_state != traffic_light.light_states[:]:
                send_to_display(f"Traffic light updated: {new_ns}")
...
# === 紧急车辆生成进程（高优先级车辆） ===
def ambulance_gen(section_queues, traffic_light, emergency_event):
    while True:
        ...
        # 通知紧急事件
        emergency_event.set()
        # 同时调用 TrafficLight.enter_emergency_mode
    ...
    def process_direction(direction):
        ...
        for v in vehicles:
            ...
            if can_pass:
                ...
                send_to_display(f"车辆 {v['license_plate']} 通过：{v['entry']} → {v['exit']} ({action})")
        return processed
...
def main():
    ...
    # 启动 display 进程
    display_process = mp.Process(target=display_server, args=(traffic_light,))
    display_process.start()

    processes = [
        mp.Process(target=light_controller, args=(traffic_light, emergency_event)),
        mp.Process(target=normal_traffic_gen, args=(section_queues,)),
        mp.Process(target=coordinator, args=(traffic_light, section_queues)),
        mp.Process(target=ambulance_gen, args=(section_queues, traffic_light, emergency_event))
    ]
